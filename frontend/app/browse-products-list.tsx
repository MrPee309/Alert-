...
/** The full, paginated product list — reached from "Gade tout pwodwi" on
 * the Pwodwi landing screen (browse-products.tsx), which shows only a
 * small "popilè" preview. This is the actual complete catalog. */
export default function BrowseProductsListScreen() {
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top", "bottom"]}>
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} testID="browse-products-list-back"><Ionicons name="chevron-back" size={26} color={colors.onSurface} /></Pressable>
        <Text style={styles.headerTitle}>Tout Pwodwi</Text>
        <View style={{ width: 26 }} />
      </View>
      ...
      <View style={styles.searchRow}>
        ...
        <TextInput
          ...
          testID="browse-products-list-search-input"
        />
      </View>
      ...
      <View style={styles.chipRow}>
        {CATEGORIES.map((c) => (
          <Pressable
            ...
            testID={`browse-products-list-cat-${c.value || "all"}`}
          >
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Pressable style={styles.card} onPress={() => router.push({ pathname: "/product-details", params: { slug: item.slug } })} testID={`browse-products-list-item-${item.id}`}>
              {item.images?.[0] ? (
              ...
              <Pressable
                ...
                testID={`browse-products-list-favorite-${item.id}`}
              >
